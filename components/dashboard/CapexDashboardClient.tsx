  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-900 md:text-3xl">🔧 CapEx Schedule</h1>
        <p className="mt-2 text-sm text-slate-600">Multi-year CapEx planning with timeline and priority tracking.</p>
      </header>
      ...
      <section className="mb-6 grid grid-cols-1 gap-4 xl:grid-cols-[300px_1fr]">
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-3 text-sm">
            <label className="block text-slate-600">Property
              <select className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-slate-800" value={propertyFilter} onChange={(e) => setPropertyFilter(e.target.value)}>
                {properties.map((item) => <option key={item}>{item}</option>)}
              </select>
            </label>
            <label className="block text-slate-600">Category
              <select className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-slate-800" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
                {categories.map((item) => <option key={item}>{item}</option>)}
              </select>
            </label>
            <label className="block text-slate-600">Year
              <select className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-slate-800" value={yearFilter} onChange={(e) => setYearFilter(e.target.value)}>
                {years.map((item) => <option key={item}>{item}</option>)}
              </select>
            </label>
            <label className="block text-slate-600">Scope
              <select className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-slate-800" value={scope} onChange={(e) => setScope(e.target.value as 'all' | 'critical' | 'overdue' | 'upcoming90')}>
      ...
      <section className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">Upcoming Major Projects (&gt;$50k / Critical)</h2>
