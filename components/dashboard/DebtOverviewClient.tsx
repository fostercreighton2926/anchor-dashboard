  ...
  return (
    <div className="px-4 py-6 md:px-8 md:py-8">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-900 md:text-3xl">🏦 Debt Overview</h1>
        <p className="mt-2 text-sm text-slate-600">Timeline visualization and refinancing priority queue</p>
      </header>
      ...
      <section className="mb-6 grid grid-cols-1 gap-4 xl:grid-cols-[300px_1fr]">
        <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-3 text-sm">
            ...
            <label className="block text-slate-600">Lender
              <select className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-slate-800" value={lenderFilter} onChange={(e) => setLenderFilter(e.target.value)}>
                {lenders.map((item) => <option key={item}>{item}</option>)}
              </select>
            </label>
            <label className="block text-slate-600">Maturity Year
            ...
            <label className="block text-slate-600">Scope
              <select className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-2 text-slate-800" value={scope} onChange={(e) => setScope(e.target.value as 'all' | 'due12' | 'troubled')}>
      ...
      <section className="overflow-x-auto rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">⚠️ Refinancing Priority Queue</h2>
