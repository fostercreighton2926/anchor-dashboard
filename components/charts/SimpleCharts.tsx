  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        {onSortToggle ? (
          <button onClick={onSortToggle} className="rounded-lg border border-slate-700 px-2 py-1 text-xs text-slate-300 hover:border-slate-500">
            Sort: {sortLabel}
          </button>
        ) : null}
      </div>
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
      ...
      <div className="mt-3 flex items-center gap-4">
        <svg width="140" height="140" viewBox="0 0 140 140" aria-label={title}>
          ...
          {donut ? <text x="70" y="74" textAnchor="middle" fill="#e2e8f0" fontSize="11">{Math.round(total)}</text> : null}
        </svg>
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-4">
      ...
      <svg className="mt-4 h-56 w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={title}>
        <line x1="0" y1="100" x2="100" y2="0" stroke="#475569" strokeDasharray="2 2" />
        {data.map((item) => {
      ...
      <p className="mt-2 text-xs text-slate-400">Diagonal line indicates DSCR break-even trend.</p>
    </div>
  )
}
        ...
        {rows.map((row) => {
          ...
          return (
            <div key={`${row.label}-${row.startMonth}`}>
              ...
              <div className="relative h-5 rounded bg-slate-800">
