...
const palette = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#0ea5e9', '#6366f1', '#14b8a6', '#f97316']
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        {onSortToggle ? (
          <button onClick={onSortToggle} className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-xs text-slate-700 hover:border-[#7A9A8A]/40 hover:bg-[#7A9A8A]/10">
            Sort: {sortLabel}
          </button>
        ) : null}
      </div>
          ...
          const row = (
            <div key={item.label} className="group">
              ...
              <div className="h-3 rounded-full bg-slate-100">
                <div className="h-full rounded-full transition-all" style={{ width, backgroundColor: item.color ?? '#3b82f6' }} />
              </div>
            </div>
          )
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <div className="mt-3 flex items-center gap-4">
        <svg width="140" height="140" viewBox="0 0 140 140" aria-label={title}>
          ...
          {donut ? <text x="70" y="74" textAnchor="middle" fill="#334155" fontSize="11">{Math.round(total)}</text> : null}
        </svg>
        <div className="space-y-2 text-xs">
          {data.map((item, index) => (
            <div key={item.label} className="flex items-center gap-2 text-slate-700">
              ...
              <span className="text-slate-500">{((item.value / total) * 100).toFixed(0)}%</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <svg className="mt-4 h-56 w-full" viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={title}>
        <line x1="0" y1="100" x2="100" y2="0" stroke="#94a3b8" strokeDasharray="2 2" />
        {data.map((item) => {
      ...
      <p className="mt-2 text-xs text-slate-500">Diagonal line indicates DSCR break-even trend.</p>
    </div>
  )
}
